...
def test_list_demos_returns_latest(client, users, taxon_ids):
    author = users["admin_user"]
    first = create_demo(author, description="List request 1", taxa_ids=taxon_ids[:2])
    second = create_demo(
        author,
    ...
    with logged_user(client, author):
        response = client.get(
            "/demo/",
            query_string={"orderby": "id_demo", "sort": "desc", "per_page": 5},
        )

    assert response.status_code == 200
    payload = response.get_json()
    ids = [item["id_demo"] for item in payload["items"]]
    assert second.id_demo == payload["items"][0]["id_demo"]
    assert first.id_demo in ids


def test_list_demos_respects_scope(client, users, taxon_ids):
    other_request = create_demo(
        users["admin_user"], description="Hidden", taxa_ids=taxon_ids[:2]
    )
    self_request = create_demo(
    ...
    with logged_user(client, users["self_user"]):
        response = client.get(
            "/demo/",
            query_string={"orderby": "id_demo", "sort": "desc", "per_page": 10},
        )

    assert response.status_code == 200
    ids = [item["id_demo"] for item in response.get_json()["items"]]
    assert self_request.id_demo in ids
    assert other_request.id_demo not in ids


def test_get_demo_returns_payload(client, users, taxon_ids, area_ids):
    created = create_demo(
        users["admin_user"],
        description="Fetch",
        taxa_ids=taxon_ids[:2],
    ...
    with logged_user(client, users["admin_user"]):
        response = client.get(f"/demo/{created.id_demo}")

    assert response.status_code == 200
    data = response.get_json()
    assert data["id_demo"] == created.id_demo
...
def test_create_demo_success(client, users, taxon_ids, area_ids):
    created_on = date.today()
    ...
    created = db.session.get(Demo, data["id_demo"])
...
def test_create_demo_rejects_invalid_taxa(client, users):
    created_on = date.today()
    payload = {
...
def test_create_demo_rejects_invalid_areas(client, users, taxon_ids):
    created_on = date.today()
    payload = {
...
def test_update_demo_updates_fields(client, users, taxon_ids, area_ids):
    created = create_demo(
        users["admin_user"],
    ...
    with logged_user(client, users["admin_user"]):
        response = client.patch(
            f"/demo/{created.id_demo}", json=update_payload
        )

    assert response.status_code == 200
    data = response.get_json()
    assert data["description"] == update_payload["description"]
    reloaded = db.session.get(Demo, created.id_demo)
...
def test_delete_demo_removes_entry(client, users, taxon_ids):
    created = create_demo(
        users["admin_user"], description="To delete", taxa_ids=taxon_ids[:2]
    )

    with logged_user(client, users["admin_user"]):
        response = client.delete(f"/demo/{created.id_demo}")

    assert response.status_code == 204
    assert db.session.get(Demo, created.id_demo) is None
...
def test_update_validated_sets_validator(client, users, taxon_ids):
    ...
    with logged_user(client, users["admin_user"]):
        response = client.patch(
            f"/demo/{created.id_demo}/validated",
            json={"validated": True},
        )
